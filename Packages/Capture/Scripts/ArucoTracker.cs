            ...
            if (m_action == null)
            {
                ...
                if (LynxOpenCV.ProcessFrameAruco(frameInfo.leftEyeBuffer, (int)frameInfo.width, (int)frameInfo.height, out outTransformation)) // OpenCV process
                {
                    m_action = () =>
                    {
                        ...
                        rotationMatrix.SetRow(0, new Vector4(outTransformation.rotationMatrix[0], outTransformation.rotationMatrix[1], outTransformation.rotationMatrix[2], 0));
                        rotationMatrix.SetRow(1, new Vector4(outTransformation.rotationMatrix[3], outTransformation.rotationMatrix[4], outTransformation.rotationMatrix[5], 0));
                        rotationMatrix.SetRow(2, new Vector4(outTransformation.rotationMatrix[6], outTransformation.rotationMatrix[7], outTransformation.rotationMatrix[8], 0));
                        ...
                        // We want to transform the rotation from opencv to unity so we put a minus sign in front of the y value. As this changes the right handedness of the coordinate system
